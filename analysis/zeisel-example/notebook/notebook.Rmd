...
saveRDS(
  cv_cov_results,
  file = here("zeisel-example", "results", "p1000_cv_cov_est_results.rds")
)
```
...
plot(cv_cov_results, dat_orig = data_in)
```

```{r estimate-comp, fig.width = 10, fig.heigt = 5}
plot(cv_cov_results, dat_orig = data_in, plot_type = "heatmap",
     estimator = c("linearShrinkLWEst", "sampleCovEst"),
     stat = c("min"), abs_v = TRUE)
plot(cv_cov_results, dat_orig = data_in, plot_type = "eigen",
